...
<div class='row'>
	...
	<div class='col-md-2' style='padding-right:0px'><?php EpicMainNav(); ?></div>
	<div id="main-sidebar-sub" class="container bg-dark sub-mobile-menu"><?php EpicSideSub(); ?></div>
	<div class='col-md-8 maincontent toppad'>
		...
		<div id="main-content-header">
			<div id="must-read-alert" class="main-alert main-alert-notice border-box" style="display:none;">
				<p>Welcome to the Must Reads! We update this page regularly with only the best content on the site. Enjoy!</p>
				<i class="icon-remove main-alert-close"></i>
			</div>
			<h1 class="page-title mike"><?php wp_title(); ?></h1>
			<div id="feed-utils-container" class="dropdown"><?php EpicDropDown(); ?></div>
	 	</div>
		...
		<?php
			...
			<?php  if(!is_user_logged_in() && get_option('epicred_ip') == 'no') { ?><!--</a>--><?php } ?>
			<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); ?>
			<?php if ( has_post_thumbnail() ) { ?><?php }else{ ?><?php } ?>
			...
			<div class = 'span5'>
				...
				<a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>">	
					...
					if($post->post_type == 'post'){
						...
						<p class="title">
							<a href="<?php echo esc_url($out); ?>" title="<?php the_title_attribute(); ?>" target="_blank" rel="nofollow">
								<?php the_title(); ?>
							</a>
							<span class='tiny grey link-domain out-link'><?php echo $n['host']; ?></span>
						</p>
					<?php }
					...
					<span class = 'tagline'>
						<?php // the_author_posts_link();?><?php $category = get_the_category(); 
						if($category[0]){
							echo '<a href="'.get_category_link($category[0]->term_id ).'">'.$category[0]->cat_name.'</a>';
						} ?>
						<?php echo human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ago'; ?> /
						<a href="<?php the_permalink(); ?>">
							<?php comments_number( 'Discuss', '1 comment', '% comments' ); ?>
						</a>
					...
					<div class='post-item-footer'>
						...
						<span class='com-ava'>
							...
							foreach($comments as $comment){
								if(!in_array($comment->user_id,$commarr)){
									...
									<span class='author-tool' data-toggle="tooltip" data-placement="bottom" title="<?php echo $comment->comment_author; ?>">
						...
						<span> <a href='#' class='small tiny link-details' data-show='<?php // echo $post->ID; ?>'>View</a></span>-->
					</div>
			...
			<div class = 'span8 pull-right'><?php comments_template(); ?></div>
			...
			<?php else: ?> 
				<p><?php _e('No post on this page yet. Want to add some?.'); ?></p> 
			<?php endif; ?>
	
		    <div class='next-posts-link'><?php echo get_next_posts_link('More Posts'); ?></div>
